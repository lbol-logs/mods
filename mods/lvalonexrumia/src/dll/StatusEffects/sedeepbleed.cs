	...
	public sealed class sedeepbleed : StatusEffect
	{
		public int truecount
		{
			get
			{
				...
				if (flag)
				{
					num = (int)Math.Ceiling((double)((float)base.Owner.MaxHp * 0.02f));
				}
				else
				{
					num = 0;
		...
		{
			base.Count = (int)Math.Ceiling((double)((float)unit.MaxHp * 0.02f));
		...
		private IEnumerable<BattleAction> OnDamageReceived(DamageEventArgs args)
		{
			...
			if (flag)
			{
				...
				yield return new ChangeLifeAction(-(int)Math.Ceiling((double)((float)args.Target.MaxHp * 0.02f)), args.Target);
			}
			yield break;
		}
